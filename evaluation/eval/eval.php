...
if (!isset($_POST['ville']) || strlen($_POST['ville']) < 2  || strlen($_POST['ville']) > 50) {
   ...
   if (!isset($_POST['prix']) || strlen($_POST['prix']) < 2 || ($_POST['prix']) > 100000000000) {
	...
	if (empty($contenu)) {
       ...
       debug($_FILES);
		if (!empty($_FILES['photo']['name'])) { 
			...
			$photo_bdd = 'photo/' .  $_FILES['photo']['name']; // chemain + nom du fichier de la photo que l'on met en BDD. ne pas oublier de créer le dossier "photo"
...
<?php echo $contenu;?><!-- affcichage echo $contenu toujours apres le "<body>" d'ouverture-->
	...
	<form method="post" enctype="multipart/form-data" > 
        ...
        <div>
            ...
            <input type="text" id="surface" name="surface" min="10" max="1000">
